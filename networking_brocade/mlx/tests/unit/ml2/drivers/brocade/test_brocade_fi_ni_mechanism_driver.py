...
from neutron.plugins.ml2.drivers.brocade.db import models as brocadedb
from neutron.plugins.ml2.drivers.brocade.fi_ni import (mechanism_brocade_fi_ni
    as brocadefinimechanism)
from neutron.tests import base
from oslo_utils import importutils

LOG = logging.getLogger(__name__)
MECHANISM_NAME = ('neutron.plugins.ml2.'
                  'drivers.brocade.fi_ni.mechanism_brocade_fi_ni.'
                  'BrocadeFiNiMechanism')
...
class TestBrocadeFiNiMechDriver(base.BaseTestCase):
    ...
    def test_create_network_precommit_wrong_physnet(self):
        """
        ...
        self.assertRaisesRegexp(Exception, "network cannot be created in the"
                                " configured ",
                                self.mechanism_driver.create_network_precommit,
                                ctx)

    def test_create_network_precommit_wrong_network_type(self):
        """
        ...
        self.assertRaisesRegexp(Exception, "only network type vlan is"
                                " supported",
                                self.mechanism_driver.create_network_precommit,
                                ctx)

    @mock.patch.object(brocadedb, 'get_network')
    ...
    def test_create_network_postcommit(self, mock_driver,
        ...
        mock_db_get_network.return_value = ctx.current
        self.mechanism_driver.create_network_postcommit(ctx)
        global config_map
        self.assertDictSupersetOf(config_map, actual_config_map)

    @mock.patch.object(brocadedb, 'delete_network')
    @mock.patch.object(brocadedb, 'get_network')
    @mock.patch.object(brocadefinimechanism.BrocadeFiNiMechanism,
                       '_get_driver')
    def test_create_network_postcommit_driver_exception(self, mock_driver,
                                                    mock_db_get_network,
                                                    mock_db_delete_network):
        """
        Expect an exception while trying to get the driver
        """
        e = Exception("Brocade FI/NI Mechanism:create_network_postcommmit"
                      " failed- Unsupported Device")
        mock_driver.side_effect = e
        ctx = self._get_network_context('physnet1', 'vlan')
        mock_db_get_network.return_value = ctx.current
        mock_db_get_network = mock.MagicMock()
        self.assertRaisesRegexp(
            Exception, "Brocade FI/NI Mechanism: "
            "create_network_postcommmit failed",
            self.mechanism_driver.create_network_postcommit,
            ctx)
