...
def upgrade():
    ...
    sa.Column('subnet_mask', sa.String(length=36), nullable=True),
    sa.PrimaryKeyConstraint('id', 'svi_id'),
    mysql_engine='InnoDB'
    )
    op.create_index(op.f('ix_ml2_brocadesvis_tenant_id'), 'ml2_brocadesvis', ['tenant_id'], unique=False)
    op.add_column('ml2_brocadeports', sa.Column('host', sa.String(length=255), nullable=True))
