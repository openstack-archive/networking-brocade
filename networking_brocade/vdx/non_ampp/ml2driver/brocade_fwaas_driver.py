...
class BrocadeFwaasDriver(fwaas_base.FwaasDriverBase):
    def __init__(self):
        LOG.debug(_("Initializing fwaas Brocade driver"))
        self._driver = None
    ...
    def brocade_init(self):
        ...
        LOG.debug(_("brocade init BrocadeFwaas Drivers"))
        ...
        LOG.debug(_("FWAAS PARAMETERS seq_ids %s direction %s count %s"
        ...
        if not ( (self._fwaas['direction'] == 'both') or\
            ...
            LOG.warning(_("invalid direction %s intializing todirection both"),
                          self._fwaas['direction'])
    ...
    def open_file_if_exists(self, fname):
        ...
        if os.path.isfile(fname):
            with open(fname, "r") as acl_file:
                try:
                    data = json.load(acl_file)
                    if 'pre_acls' in data:
                        pre_acls = data['pre_acls']
                    if 'post_acls' in data:
                        post_acls = data['post_acls']
                    LOG.debug(_("pre acls : %s"), pre_acls)
                    LOG.debug(_("post acls : %s"), post_acls)
                except  Exception as e:
                    LOG.warning(_("Error Loadng %s file(may be empty file)"),
                                  fname)
                    return pre_acls, post_acls
        else:
            LOG.warning(_("%s file doesn't exists"), fname)
    ...
    def _update_firewall(self, apply_list, firewall):
        ...
        try:
            ...
            LOG.error(_("_update_firewall::Error creating ACL policy :"
                        "Error: %s"), e)
            raise e
        return True

    def _apply_policy_on_interface(self, policy_name, svi):
        LOG.debug(_("brocade_fwaas:_setup_policy svi %s"), svi)
    ...
    def merge_and_replay_acls(self, name):
        if self.req:
            ...
            LOG.debug(_("merge_and_replay_acls netconf %s"), acl_netconf)
    ...
    def _config_replay_acls(self, policy_name, rule, seq_id):
        try:
            ...
            LOG.error(_("error creating rule %s"), e)
            raise e
        return
    ...
    def _config_replay_acls_file(self, policy_name, acl_file, seq_ids, index):
        for rule in  acl_file:
            rule = rule['acl']
            try:
                self._config_replay_acls(policy_name, rule,str(seq_ids[index]))
            except Exception as e:
                 LOG.error(_("error _config_replay_acls_file %s"), e)
                 raise e
            index = index + 1
        return index
    ...
    def _setup_policy(self, apply_list, fw):
        ...
        try:
            ...
            LOG.error(_("Error creating ACL policy :Error: %s"), e)
    ...
    def _clear_policy(self, apply_list, fw):
        ...
        for ri in apply_list:
            for svi in ri.router['svis']:
                LOG.debug(_("brocade_fwaas:_clear_policy svi %s"), svi)
                if(self._fwaas['direction'] == 'both' or\
                   self._fwaas['direction'] == 'in'):
                    for rbridge_id in self._switch['rbridge_ids']:
    ...
    def _make_policy(self, name, rule, seq_id):
        ...
        if((protocol == 'any') or (protocol == None)):
            protocol = 'ip'
        ...
        try:
            xml_request = utils.make_rule(name, seq_id, action, protocol, src_ip,
            ...
            LOG.error(_("error _make_policy %s"), e)
            raise e

        LOG.debug(_("xml request %s"), xml_request)
