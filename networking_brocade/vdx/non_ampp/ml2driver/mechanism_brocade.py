...
class BrocadeMechanism(api.MechanismDriver):
    ...
    def create_port_precommit(self, mech_context):
        ...
        self._create_brocade_port(context, port, mech_context.top_bound_segment)
    ...
    def _is_compute_or_dhcp_port(self, port, context):
        if (("compute" not in port['device_owner'])
            and ("dhcp" not in port['device_owner'])):
        ...
        for (speed, name) in self._device_dict[(hostname,physical_network)]:
            ...
            try:
                if not brocade_db.is_vm_exists_on_host(context,
                    hostname, physical_network, vlan_id):
        ...
        for (speed, name) in self._device_dict[(hostname,physical_network)]:
            try:
                if brocade_db.is_last_vm_on_host(context, 
                    ...
                    LOG.info(_("more vm exist for network on host hence vlan is"
                               "not removed from port"))
            except Exception:
                LOG.exception(
