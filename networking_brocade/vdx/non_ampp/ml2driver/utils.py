...
RBRIDGE_OPTS =  [cfg.ListOpt('rbridge_id', default = DEFAULT_RBRIDGE_RANGE,
                              help=_('rbridges range 1,2,3' )),
                 cfg.StrOpt('redundency', default='disabled',
                            help=_('enable/disable L3 redundency')),
                 ...
                 cfg.IntOpt('vrrp_advertisement_intervel', default=1,
                            help=_('vrrp advertisement intervel'))
                ]
...
def get_brocade_l3_config():
    ...
    svi = {'rbridge_ids':cfg.CONF.svi.rbridge_id,
           'redundency':True if ((cfg.CONF.svi.redundency == 'enabled') and\
           (len(cfg.CONF.svi.rbridge_id) > 1) ) else False,
           'vrrp_version':cfg.CONF.svi.vrrp_version,
           'vrrp_group_id':cfg.CONF.svi.vrrp_group_id,
           'vrrp_advertisement_intervel':\
                cfg.CONF.svi.vrrp_advertisement_intervel,
           'is_vrf_required':cfg.CONF.svi.is_vrf_required
          }

    if not svi['redundency'] and len(svi['rbridge_ids']) > 1:
        #redundency disabled consider only first rbride configured
        del svi['rbridge_ids'][1:]
    LOG.info(_("rbridge_ids %s redundency %s vrrp_version %s"
               " vrrp_group_id %s"), svi['rbridge_ids'],
               svi['redundency'], svi['vrrp_version'],
               svi['vrrp_group_id'])
...
class SeqIdBitmap(object):
    ...
    def get_seq_ids(self, acl, howmany):
        ...
        ids = []
        it = 0
        for seq_id in moves.xrange(self._min_search_seqid,
...
def get_port_operator(port_low, port_high):
   """detect if user has entered single port
      or range of ports"""
        ...
        if len(topology[host_physnet]) >= 1:
            for item in topology[host_physnet]:
                if ((not bond_info)  and (po_hi >= po_lo)):
                    ...
                    LOG.debug(_("po lo %d po_hi %d"), po_lo, po_hi)
                elif bond_info:
                    #only one port-channel
                    po_lo = bond_info[host_physnet][0]
                    lacp_ports.setdefault(po_lo, []).append(item)
                else:
                    LOG.error(_("exhausted all port-channels increase"
                    "port-channel range or bond mappings not provided"))
                    sys.exit(0)
            ...
            if not bond_info:
                po_lo = po_lo + 1
    LOG.debug(_("device_dict %s lacp_ports %s"),device_dict, lacp_ports)
...
def _is_valid_three_tupple(interface):
   ...
   if '/' in interface:
       ...
       if len(s) != 3 :
           LOG.error(_("_is_valid_three_tupple:invalid interface %s configure"
...
def _is_valid_interface_speed(speed):
   ...
   if 'ten' in speed:
       ...
       LOG.error(_("_is_valid_interface_speed:invalid speed parameter %s"
