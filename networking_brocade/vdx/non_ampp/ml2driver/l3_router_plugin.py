...
class BrocadeSVIPlugin(router.L3RouterPlugin):
    ...
    def _update_ips_for_port(self, context, port):
        ...
        LOG.info(_("_update_ips_for_port generated ip %s"), result)
    ...
    def _invoke_nos_driver_api(self, func_name, router_id,
			       ...
			       added = None,
			       removed = None):
        LOG.info(_("_invoke_nos_driver_api called()"))
        switch = self._switch
        priority = 1
        if  self._svi['redundency'] and\
            (func_name == 'delete_svi' or\
             func_name == 'create_svi'):
            vip, net_len =  self.net_addr(gateway_ip_cidr)
            LOG.info(_("_invoke_nos_driver_api vip %s net_len %s"), vip,
                                                                  net_len)
        ...
        for rbridge_id in self._svi['rbridge_ids']:
            if func_name == 'create_router' or\
               ...
               if self._svi['redundency']:
                   ...
                   vrrp_advt_intervel =\
                       self._svi['vrrp_advertisement_intervel']
                   try:
    ...
    def create_router(self, context,router):
        ...
        try:
            ...
            LOG.error(_("Failed to create router Reason %s"), str(e))
            raise e
        return new_router
	...
	try:
	    ...
	    LOG.error(_("Failed to modify route %s"), str(e))
	    raise e
	return updated_router

    def delete_router(self, context, router_id):
        """ delete a vrf on NOS device """
        LOG.debug(_("RouterMixin.delete_router() called, "
        ...
        try:
           self._invoke_nos_driver_api("delete_router", router['id'])
            ...
            LOG.error(_("Failed to delete router Reason %s"), str(e))
            raise e

    def add_router_interface(self, context, router_id, interface_info):
        """creates svi on NOS device and assigns ip addres to SVI"""
        LOG.debug(_("BrocadeSVIPlugin.add_router_interface called: "
        ...
        try:
            ...
            LOG.debug(_("Allocated cidr (%s) from the pool, network_id(%s)"
            ...
            port_count = self._core_plugin.get_ports_count(context,
                                                           port_filters)
            LOG.info(_("BrocadeSVIPlugin.add_router_interface"
            ...
            if port_count == 2:
                ...
                LOG.error(_("BrocadeSVIPlugin:adding redundent router"
    ...
    def remove_router_interface(self, context, router_id, interface_info):
        ...
        LOG.debug(_("BrocadeSVIPlugin.remove_router_interface called: "
                    "router_id=%(router_id)s "
                    "interface_info=%(interface_info)r"),
                  {'router_id': router_id, 'interface_info': interface_info})
        ...
        try:
            ...
            vlan_id = bnet['vlan']
            gateway_ip_cidr=gateway_ip+'/'+str(net_len)
            LOG.debug(_("remove_router_interface removed cidr (%s)"
            ...
            with excutils.save_and_reraise_exception():
                LOG.error(_("Failed to remove interface from brocade router"
                        "interface. info=%(info)s, router_id=%(router_id)s"),
                      {"info": info, "router_id": router_id})
    ...
    def _update_firewall(self, context, svi,tenant_id):
        ...
        if not fw_plugin:
            LOG.info(_('No Firewall plugin registered!!'))
            return
        context.tenant_id = tenant_id
        if hasattr(fw_plugin, 'handle_router_interface_add'):
            fw_plugin.handle_router_interface_add(context, svi,tenant_id)
        else:
            LOG.warning(_("Brocade SVI Plugin is used but brocade firewall"
            " plugin you may want to configure brocade firewall plugin"))
